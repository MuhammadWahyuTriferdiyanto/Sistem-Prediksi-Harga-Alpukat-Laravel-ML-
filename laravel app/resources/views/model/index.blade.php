@extends('layouts.app')
@section('title','Status Model ML')
@section('content')

<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
  <h5 class="fw-bold mb-0">Status Model Machine Learning</h5>
  <span class="ml-status {{ $status['online'] ? 'online' : 'offline' }}">
    <span class="dot"></span> Service {{ $status['online'] ? 'Online' : 'Offline' }}
  </span>
</div>

@if(session('metrics_baru'))
<div class="alert alert-success">
  <i class="fas fa-check-circle me-2"></i>Model baru selesai dilatih. Akurasi (R²): <strong>{{ session('metrics_baru')['r2_score'] ?? '-' }}</strong>
</div>
@endif
    ...
    <div class="card-kampung card">
      ...
      <div class="card-body p-4">
        ...
        <div class="row g-3 mb-4">
          <div class="col-md-4">
            <div class="p-3 rounded text-center" style="background:#eef2ff">
              ...
              <h4 class="fw-bold text-primary mb-0">{{ $metrics['r2_score'] }}</h4>
          ...
          <div class="col-md-4">
            <div class="p-3 rounded text-center" style="background:#fef3c7">
              ...
              <h4 class="fw-bold mb-0" style="color:#92400e">{{ $metrics['mape_percent'] }}%</h4>
          ...
          <div class="col-md-4">
            <div class="p-3 rounded text-center" style="background:#dcfce7">
              ...
              <h4 class="fw-bold text-success mb-0">Rp {{ number_format($metrics['mae'],0,',','.') }}</h4>
        ...
        <table class="table table-sm">
          <tr><td class="text-muted" style="width:200px">Jumlah data training</td><td>{{ $metrics['n_train'] }} baris</td></tr>
          <tr><td class="text-muted">Jumlah data testing</td><td>{{ $metrics['n_test'] }} baris</td></tr>
          <tr><td class="text-muted">Fitur yang digunakan</td><td><code>{{ implode(', ', $metrics['features']) }}</code></td></tr>
          <tr><td class="text-muted">Jenis alpukat dikenali</td><td>{{ implode(', ', $metrics['jenis_alpukat_classes']) }}</td></tr>
        </table>
        @else
  ...
  <div class="col-lg-4">
    <div class="card-kampung card">
      ...
      <div class="card-body p-4">
        ...
        <form method="POST" action="{{ route('model.latih-ulang') }}" onsubmit="return confirm('Latih ulang model? Proses ini mungkin memakan waktu beberapa saat.')">
          @csrf
          <button type="submit" class="btn btn-accent w-100" {{ !$status['online'] ? 'disabled' : '' }}>
