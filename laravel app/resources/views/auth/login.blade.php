...
<div class="login-wrapper">
  <div class="login-card">
    ...
    @if($errors->any())
    <div class="alert alert-danger small"><i class="fas fa-exclamation-circle me-1"></i>{{ $errors->first() }}</div>
    @endif
    @if(session('success'))
    <div class="alert alert-success small"><i class="fas fa-check me-1"></i>{{ session('success') }}</div>
    @endif
    ...
    <form method="POST" action="{{ route('auth.login') }}">
      ...
      <div class="mb-3">
        ...
        <input type="email" name="email" class="form-control" value="{{ old('email') }}" placeholder="admin@prediksialpukat.id" required>
      </div>
      <div class="mb-3">
